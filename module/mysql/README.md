...
| `bastion_ssh_key_name` | `my-rds-testing-key` | Name of an existing EC2 key pair in the target region |
...
```bash
cd module/mysql/environments/<env>/<region>
terragrunt apply
```
...
| `bastion_ssh_tunnel_command` | Ready-to-run SSH command that forwards `localhost:3306` to the RDS endpoint |
