...
locals {
  ...
  # Create one: aws ec2 create-key-pair --key-name my-rds-key --region us-east-1 --query KeyMaterial --output text > ~/.ssh/my-rds-key.pem && chmod 400 ~/.ssh/my-rds-key.pem
  bastion_ssh_key_name = "my-rds-key"
